...
describe Gcream::Rule::ConsecutiveGrowthRule do
  context "initialized growth rule" do
    let!(:values)    { [5, 4, 3, 2, 0] }
    let!(:statement) { double(eps: values) }

    subject { Gcream::Rule::ConsecutiveGrowthRule.new(statement, :eps, nil, "Quarters") }
...
describe Gcream::Rule::ConsecutiveDeclineRule do
  context "initialized growth rule" do
    let!(:values)    { [3, 4, 5, 2, 0] }
    let!(:statement) { double(eps: values) }

    subject { Gcream::Rule::ConsecutiveDeclineRule.new(statement, :eps, nil, "Quarters") }
    ...
    describe "#valid?" do
      ...
      context "a rule value that fails against the values" do
        subject { Gcream::Rule::ConsecutiveDeclineRule.new(statement, :eps, 6, "Quarters") }
